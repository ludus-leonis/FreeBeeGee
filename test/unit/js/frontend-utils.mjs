...
describe('Frontend - utils.mjs - Arrays', function () {
  ...
  it('mode()', function () {
    expect(mode([1, 1, 1])).to.be.eql(1)
    expect(mode([1, 2, 1])).to.be.eql(1)
    expect(mode([2, 2, 1])).to.be.eql(2)
    expect(mode([1, 2, 2])).to.be.eql(2)
    expect(mode([2, 1, 2])).to.be.eql(2)
    expect(mode([1, 2, 3, 3, 2, 1])).to.be.eql(1) // equal = last
    expect(mode([true, 0, true, 'foo'])).to.be.eql(true)
    expect(mode([])).to.be.eql(undefined)
    expect(mode()).to.be.eql(undefined)
  })
})
