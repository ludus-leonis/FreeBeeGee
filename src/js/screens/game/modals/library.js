...
import { stateCreatePiece, getLibrary, stateGetGamePref, stateSetGamePref } from '../state.js'
import { getMaxZ, pieceToNode, nodeToPiece } from '..'
  ...
  if (!modalActive()) {
    ...
    getLibrary()
      .then(library => {
        ...
        for (const asset of library.tile) { tiles.add(assetToNode(asset)) }
        const overlays = _('#tab-overlays')
        for (const asset of library.overlay) { overlays.add(assetToNode(asset)) }
        const tokens = _('#tab-tokens')
        for (const asset of library.token) { tokens.add(assetToNode(asset)) }
      ...
      .catch(error => {
        document.getElementById('modal-body').innerHTML = `
          <p>Sorry, could not load assets: ${error.message}.</p>
        `
      })
  }
}
 ...
 *
 * @param {Object} asset The piece to convert.
 * @return {HTMLElement} Node for the modal.
 */
function assetToNode (asset) {
  const piece = pieceToNode(asset).add(
    asset.type === 'tile' ? '.is-scale-2' : '.is-scale-2',
    '.is-w-' + asset.width + '.is-h-' + asset.height
  )
  const card = _('.col-6.col-sm-4.col-md-3.col-lg-2.col-card').create(piece)
  if (asset.assets.length > 1) {
    piece.add(_('.tag.tr').create().add(document.createTextNode(
      `${asset.width ?? 1}x${asset.height ?? 1}:${asset.assets.length ?? 0}`
    )))
  } else {
    piece.add(_('.tag.tr').create().add(document.createTextNode(
      `${asset.width ?? 1}x${asset.height ?? 1}`
    )))
  }
  card.add(_('p').create().add(document.createTextNode(
    `${prettyName(asset.alias)}`
  )))
  return card
}
